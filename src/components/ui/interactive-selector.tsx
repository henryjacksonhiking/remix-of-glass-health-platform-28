    ...
    <div className="w-full max-w-6xl mx-auto">
      ...
      <div className="flex gap-2 md:gap-3 h-[420px] md:h-[480px]">
        {options.map((option, index) => {
          const Wrapper = option.slug ? Link : 'div';
          const wrapperProps = option.slug ? { to: `/blog/${option.slug}` } : {};
          return (
            <Wrapper
              key={index}
              {...(wrapperProps as any)}
              className={`relative rounded-2xl overflow-hidden cursor-pointer transition-all duration-700 ease-in-out block ${
