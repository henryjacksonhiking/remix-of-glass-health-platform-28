    ...
    <PageWrapper>
      ...
      <div className="container mx-auto px-4 md:px-6 pt-6">
        <nav aria-label="breadcrumb" className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span style={{ color: '#1435C1' }}>Borna Insight</span>
        </nav>
      </div>
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          <div className="max-w-3xl mx-auto text-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 rounded-full border border-glass-border bg-glass px-4 py-1.5 text-sm text-muted-foreground mb-6">
              ...
              <span style={{ color: '#1435C1' }}>Analytics</span>
            </motion.div>
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="hero-headline text-foreground mb-6">
              From raw data to <span className="bg-clip-text text-transparent" style={{ backgroundImage: 'linear-gradient(135deg, #1435C1, #00DEC4)' }}>actionable intelligence</span>
            </motion.h1>
      ...
      <section className="py-12 md:py-20 border-t border-glass-border">
        <div className="container mx-auto px-4 md:px-6 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3 max-w-2xl mx-auto">
            {["Scheduling data", "Billing records", "Call logs", "Patient forms", "EHR exports", "Email metrics"].map(item => (
              <span key={item} className="px-4 py-2 rounded-full text-sm" style={{ background: 'rgba(255,255,255,0.04)', border: '0.5px solid rgba(255,255,255,0.12)', color: 'rgba(255,255,255,0.5)' }}>
                {item}
              </span>
            ))}
          </div>
          <p className="text-xs mt-6" style={{ color: 'rgba(255,255,255,0.4)' }}>24 data points. 0 insights.</p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              <motion.div key={alert.label} initial={{ opacity: 0, x: -12 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="flex items-center gap-3 px-4 py-3 rounded-xl" style={{ background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.12)' }}>
                <Bot className="w-4 h-4 shrink-0" style={{ color: '#1435C1' }} />
                <div>
